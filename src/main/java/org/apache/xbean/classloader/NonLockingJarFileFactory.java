 ...
 */
public class NonLockingJarFileFactory implements JarFileFactory {
	
	private ConcurrentMap<File, SoftReference<NonLockingJarFile>> jarFileCache = new ConcurrentHashMap<File, SoftReference<NonLockingJarFile>>();
	...
	public JarFile newJarFile(File file) throws IOException {
		...
		if (rtn == null) {
			...
			rtn.setCheckForUpdates(checkForUpdates);
